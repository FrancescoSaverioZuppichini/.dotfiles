...
function createRequest({ sourceSchema, sourceParentType, sourceFieldName, fragments, variableDefinitions, variableValues, targetOperationName, targetOperation, targetFieldName, selectionSet, fieldNodes, }) {
    ...
    if (selectionSet != null) {
        ...
        if (args) {
            argumentNodeMap = args.reduce((prev, curr) => (Object.assign(Object.assign({}, prev), { [curr.name.value]: curr })), argumentNodeMap);
        }
    }
    ...
    const newDefinitions = operations.map(operation => {
        ...
        Object.keys(fields).forEach(responseKey => {
            ...
            fieldNodes.forEach(fieldNode => {
                ...
                newSelections.push(Object.assign(Object.assign({}, fieldNode), { selectionSet: newSelectionSet }));
            });
        });
        return Object.assign(Object.assign({}, operation), { selectionSet: {
                kind: graphql.Kind.SELECTION_SET,
...
function visitSelectionSet(node, typeInfo, selectionSetsByType, selectionSetsByField, dynamicSelectionSetsByField) {
    ...
    if (parentType != null) {
        ...
        return Object.assign(Object.assign({}, node), { selections: Array.from(newSelections.values()) });
    }
}
...
function expandAbstractTypes(targetSchema, mapping, reverseMapping, document) {
    ...
    const newDocument = Object.assign(Object.assign({}, document), { definitions: [...operations, ...newFragments] });
...
class FilterToSchema {
    ...
    transformRequest(originalRequest) {
        return Object.assign(Object.assign({}, originalRequest), filterToSchema(this.targetSchema, originalRequest.document, originalRequest.variables));
    }
}
function filterToSchema(targetSchema, document, variables) {
    ...
    const validFragmentsWithType = validFragments.reduce((prev, fragment) => (Object.assign(Object.assign({}, prev), { [fragment.name.value]: targetSchema.getType(fragment.typeCondition.name.value) })), {});
                ...
                if (graphql.isObjectType(parentType) || graphql.isInterfaceType(parentType)) {
                    ...
                    if (node.arguments != null) {
                        ...
                        if (args.length !== node.arguments.length) {
                            return Object.assign(Object.assign({}, node), { arguments: args });
                        }
                    }
                }
            },
...
class AddArgumentsAsVariables {
    constructor(targetSchema, args) {
        this.targetSchema = targetSchema;
        this.args = Object.entries(args).reduce((prev, [key, val]) => (Object.assign(Object.assign({}, prev), { [key]: val })), {});
    }
    transformRequest(originalRequest) {
    ...
    const newOperations = operations.map((operation) => {
        ...
        operation.selectionSet.selections.forEach((selection) => {
            if (selection.kind === graphql.Kind.FIELD) {
                ...
                const argumentNodeMap = argumentNodes.reduce((prev, argument) => (Object.assign(Object.assign({}, prev), { [argument.name.value]: argument })), {});
                ...
                newSelectionSet.push(Object.assign(Object.assign({}, selection), { arguments: Object.keys(argumentNodeMap).map(argName => argumentNodeMap[argName]) }));
            }
            else {
                newSelectionSet.push(selection);
            }
        });
        return Object.assign(Object.assign({}, operation), { variableDefinitions: Object.keys(variableDefinitionMap).map(varName => variableDefinitionMap[varName]), selectionSet: {
                kind: graphql.Kind.SELECTION_SET,
                selections: newSelectionSet,
            } });
    });
    return {
        document: Object.assign(Object.assign({}, document), { definitions: [...newOperations, ...fragments] }),
...
function delegateToSchema(options) {
    ...
    return delegateRequest(Object.assign(Object.assign({}, options), { request,
        operation,
        fieldName,
        returnType }));
}
